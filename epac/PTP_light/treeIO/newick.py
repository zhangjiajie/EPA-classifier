...
def format_node(node, node_type, format):
    ...
    if converterFn1 == str:
        ...
        try:
            FIRST_PART =  "%0.11f" %(converterFn2(getattr(node, container1)))
    ...
    if converterFn2 == str:
        ...
        try:
            SECOND_PART = ":%0.11f" %(converterFn2(getattr(node, container2)))
...
def node2leafformat(node, format):
    ...
    if format == 0 or format == 1 or format == 2 or format ==3:
        return "%s:%0.11f" %(safe_name, node.dist)
    elif format == 4 or format == 7:
        return ":%0.11f" %(node.dist)
...
def node2internalformat(node, format):
    ...
    if format == 0 or format == 1:
        return "%0.11f:%0.11f" %(node.support, node.dist)
    elif format == 2:
        return "%s:%0.11f" %(safe_name, node.dist)
